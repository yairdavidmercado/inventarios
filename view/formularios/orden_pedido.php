...
<html lang="en">
  <head>
    ...
    <!-- Bootstrap core CSS -->
<link href="/inventarios/assets/css/bootstrap.min.css" rel="stylesheet" crossorigin="anonymous">
        ...
        <div class="row">
            <div class="col-sm-9">
                <div class="card">
                    ...
                    <a  href="ver_orden_pedido.php" class="btn btn-sm float-right" style="background-color: #6c63ff; color:#fff">Ver pedidos</a>
                        <h5 class="mb-0">Orden de pedido</h5>
                    </div>
                    <div class="card-body">
                        <div class="card mb-4">
                            <div class="card-body">
                                <form class="form" id="form_detalle_orden" role="form" onsubmit="event.preventDefault(); return guardar_detalle_pedido();" autocomplete="off">
                                    <div class="container">
                                        <div class="row">
                        ...
                        <form class="form" id="form_orden" role="form" onsubmit="event.preventDefault(); return guardar_orden_pedidos();" autocomplete="off">
                            <div class="row">
                                <div class="form-group col-sm-3">
                                ...
                                <div class="col-sm-9" style="display:none">
                                    ...
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input id="debit" value="pedido" onclick="datos_clientes('0')" name="tipo_venta" type="radio" class="custom-control-input" checked="" required="">
                                ...
                                <div id="btn_guardar" class="col-sm-12">
                                    <button type="submit" class="btn btn-success float-right">orden sin despachar</button>
                                </div>
                                <div style="display:none" id="btn_guardar_credito" class="col-sm-12">
                                    <button type="submit" class="btn btn-success float-right">Guardar orden sin despachar</button>
                                </div>
                            </div>
                        </form>
                    </div>
...
<script src="/inventarios/assets/js/jquery.slim.min.js" crossorigin="anonymous"></script>
    <script>window.jQuery || document.write('<script src="/inventarios/assets/js/jquery.slim.min.js"><\/script>')</script>
    <script src="/inventarios/assets/js/popper.min.js" crossorigin="anonymous"></script>
    <script src="/inventarios/assets/js/ajaxJquery.min.js"></script>
    ...
    <script src="/inventarios/assets/js/boot4alert.min.js" crossorigin="anonymous"></script>
    <script src="/inventarios/assets/js/jquery.dataTables.min.js"></script>
    <script src="/inventarios/assets/js/dataTables.bootstrap4.min.js"></script>
    <script src="/inventarios/assets/js/bootstrap.min.js" crossorigin="anonymous"></script>
...
$(function() {
  ...
  function guardar_detalle_pedido() {
    ...
    function eliminar_detalle_orden(id) {
        let values = {
            id: id
        }
        $.ajax({
        type : 'POST',
        data: values,
        url: '/inventarios/php/orden_pedidos/eliminar.php',
        success: function(respuesta) {
          let obj = JSON.parse(respuesta)
          if (obj.numero == 1) {
    ...
    function Showventa() {
        ...
        type : 'POST',
        data: values,
        url: '/inventarios/php/orden_pedidos/seleccionar.php',
        beforeSend: function() {
  ...
  function guardar_orden_pedidos() {
    ...
    if (productos.length == 0) {
        notificacion("No existe producto agregado a la lista para generar la orden.", "success")
        return false
    }
      $.ajax({
        type : 'POST',
        data: values,
        url: '/inventarios/php/orden_pedidos/guardar_orden_pedidos.php',
        success: function(respuesta) {
          let obj = JSON.parse(respuesta)
          if (obj.success) {
            notificacion("La orden de pedido se ha agregado exitosamente.", "success")
            //limpiar_form()
            Showventa()
            $("input[name*='id_producto']").focus()
          }else{
            notificacion("La orden de pedido ya se encuentra agregado.", "danger")
          }
        },
        error: function(e) {
  ...
  function buscar_clientes(param) {
    let values = { 
        ...
        type : 'POST',
        data: values,
        url: '/inventarios/php/cliente/seleccionar.php',
        beforeSend: function() {
            //$(".loader").css("display", "inline-block")
        },
        success: function(respuesta) {
  ...
  function datos_clientes(id) {
    if (id == "0") {
            $("#content_cliente").html('<div class="form-group col-sm-12">'+
                                            '<div class="d-block">'+
                                                '<label style="font-size:14px;" for="">Datos del cliente</label>'+
                                                '<div class="row">'+
                                                    '<div class="col-sm-3" style="display:none">'+
                                                        '<input class="form-control form-control-sm" type="text" value="0" placeholder="Número de cuotas" name="cuotas">'+
                                                    '</div>'+
                                                    '<div class="col-sm-6">'+
                                                        '<select class="form-control form-control-sm select2-single id_cliente" name="id_cliente">'+
                                                            '<option value="">Seleccione el id_cliente</option>'+
                                                        '</select>'+
                                                    '</div>'+
                                                    '<div class="col-sm-6">'+
                                                        '<a style="font-size:14px;" href="/inventarios/view/formularios/cliente.php" class="">Crear nuevo cliente</a>'+
                                                    '</div>'+
                                                '</div>'+
                                            '</div>'+
                                        '</div>')
            ...
            $("#content_cliente").html('<div class="form-group col-sm-12 py-3">'+
                                            '<div class="d-block">'+
                                                ...
                                                '<div class="row">'+
                                                    '<div class="col-sm-3" style="display:none">'+
                                                        '<input class="form-control form-control-sm" type="text" value="0" placeholder="Número de cuotas" required name="cuotas">'+
                                                    '</div>'+
                                                    '<div class="col-sm-6">'+
